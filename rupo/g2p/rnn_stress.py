...
class RNNStressPredictor:
    ...
    def build(self) -> None:
        ...
        """
        model = Sequential()
        model.add(Bidirectional(self.rnn(self.units), input_shape=(None, len(self.phonetic_alphabet))))
    ...
    def train(self, dir_name: str, enable_checkpoints: bool=False) -> None:
        ...
        # Сохранение модели.
        filename = "stress_{language}_{rnn}{units}_dropout{dropout}_acc{acc}.h5"
    ...
    def predict(self, words: List[str]) -> List[int]:
        ...
        stresses = [int(np.argmax(prob)) for prob in self.model.predict(x, verbose=0)]
        return stresses
    ...
    def __prepare_data(self, x: List[str], y: List[int]=None) -> Tuple[np.array, List[int]]:
        """
        Подготовка данных
